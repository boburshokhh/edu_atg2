...
-- Политика: администраторы могут видеть всех пользователей
...
-- Включение RLS для user_sessions
ALTER TABLE user_sessions ENABLE ROW LEVEL SECURITY;

-- Политика: пользователи видят только свои сессии
DROP POLICY IF EXISTS "Users can view own sessions" ON user_sessions;
CREATE POLICY "Users can view own sessions" ON user_sessions
    FOR SELECT
    USING (user_id::text = auth.uid()::text);

-- Включение RLS для user_profiles
ALTER TABLE user_profiles ENABLE ROW LEVEL SECURITY;

-- Политика: пользователи видят и обновляют только свой профиль
DROP POLICY IF EXISTS "Users can manage own profile" ON user_profiles;
CREATE POLICY "Users can manage own profile" ON user_profiles
    FOR ALL
    USING (id::text = auth.uid()::text);

-- Включение RLS для user_courses
ALTER TABLE user_courses ENABLE ROW LEVEL SECURITY;

-- Политика: пользователи видят только свои курсы
DROP POLICY IF EXISTS "Users can view own courses" ON user_courses;
CREATE POLICY "Users can view own courses" ON user_courses
    FOR SELECT
    USING (user_id::text = auth.uid()::text);

-- Политика: пользователи могут создавать записи о своих курсах
...
-- Политика: пользователи могут обновлять свои курсы
...
-- Включение RLS для certificates
ALTER TABLE certificates ENABLE ROW LEVEL SECURITY;

-- Политика: пользователи видят только свои сертификаты
