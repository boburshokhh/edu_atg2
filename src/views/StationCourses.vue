<template>
  <AppLayout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-1">
            <div class="sticky top-28 space-y-6">
              ...
              <!-- Enrollment Card -->
              <div class="bg-white rounded-2xl shadow-lg border-2 border-blue-500 p-6 overflow-hidden relative">
                <div class="absolute top-0 right-0 w-32 h-32 bg-blue-500 opacity-10 rounded-full -mr-16 -mt-16"></div>
                <div class="relative">
                  <button 
                    @click="startLearning"
                    class="w-full px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all mb-6 shadow-lg hover:shadow-xl transform hover:scale-105 duration-200"
                  >
                    <PlayCircleIcon class="w-5 h-5 mr-2 inline" />
                    Начать обучение
                  </button>
                  ...
                  <div class="space-y-4">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="font-bold text-gray-900">Онлайн</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <span class="font-bold text-gray-900">Навсегда</span>
                    </div>
                    <div class="h-px bg-gray-200"></div>
    ...
    <div 
      ...
      @selectstart.prevent
    >
      <div class="relative w-full h-full max-w-7xl mx-4 my-4 bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <!-- Header -->
        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-200">
        ...
        <div class="flex-1 overflow-y-auto overflow-x-hidden relative pdf-viewer-container bg-gray-100" @contextmenu.prevent @dragstart.prevent @copy.prevent @cut.prevent @keydown.prevent="handleKeyDown">
          <div v-if="currentPdf && pdfSource && pdfPages.length > 0" class="w-full flex flex-col items-center py-6 px-4 space-y-6">
            <div
              ...
              class="pdf-page w-full max-w-4xl bg-white shadow-2xl rounded-lg overflow-hidden p-2"
            >
              <canvas 
        ...
        <!-- Footer Info -->
        <div class="flex items-center justify-center p-4 bg-gray-50 border-t border-gray-200">
...
export default {
  ...
  setup() {
    ...
    const pdfPages = ref([]) // Массив canvas элементов для каждой страницы
    const pdfDoc = ref(null) // PDF документ
    const canvasRefs = ref([]) // Референсы к canvas элементам
    
    const pdfSource = computed(() => {
      if (!currentPdf.value) return null
      return currentPdf.value.url || currentPdf.value.file_url || ''
    })

    // Установка ref для canvas
    ...
    const handleFileClick = (file, allFiles, fileIndex) => {
      ...
      if (isVideoFile(file)) {
        ...
        // Для PDF открываем в просмотрщике, а не скачиваем
        openPdfViewer(file)
      }
    }

    // Открыть PDF в просмотрщике
    const openPdfViewer = async (file) => {
      ...
      if (!file || (!file.url && !file.file_url)) {
        ElMessage.warning('Файл недоступен для просмотра')
        return
      }
      ...
      // Блокируем контекстное меню и выделение текста
    ...
    // Загрузка PDF документа и отрисовка всех страниц
    const loadPdfDocument = async () => {
      if (!pdfSource.value) return
      try {
        // Загружаем PDF документ через сервис
        const pdf = await pdfService.loadPdfDocument(pdfSource.value)
        pdfDoc.value = pdf
        totalPages.value = pdf.numPages
        
        // Создаем массив для страниц
        pdfPages.value = new Array(totalPages.value).fill(null)
        
        // Ждем следующего тика, чтобы Vue создал все canvas элементы
        await nextTick()
        
        // Загружаем и отрисовываем все страницы
    ...
    // Отрисовка всех страниц PDF
    const renderAllPages = async (pdf) => {
      if (!pdf) pdf = pdfDoc.value
      if (!pdf || !totalPages.value) return
      
      // Ждем, пока Vue создаст все canvas элементы
      await nextTick()
      // Дополнительная задержка для гарантии создания всех элементов
      await new Promise(resolve => setTimeout(resolve, 300))
      
      // Отрисовываем каждую страницу последовательно
      for (let pageNum = 1; pageNum <= totalPages.value; pageNum++) {
        try {
          // Получаем страницу через сервис
          const page = await pdfService.getPdfPage(pdf, pageNum)
          
          // Получаем canvas элемент для этой страницы
          let canvas = canvasRefs.value[pageNum - 1]
          
          // Если canvas еще не готов, ждем
          let retries = 0
          ...
          if (!canvas) {
            console.warn(`Не удалось найти canvas для страницы ${pageNum}`)
            continue
          }
          
          // Вычисляем оптимальный масштаб для canvas
          const scale = pdfService.calculateOptimalScale(page, 1200)
          
          // Отрисовываем страницу через сервис
          await pdfService.renderPdfPage(page, canvas, scale)
        } catch (error) {
          console.error(`Ошибка отрисовки страницы ${pageNum}:`, error)
        }
      }
    }
    ...
    // Блокировка комбинаций клавиш для защиты контента
    const handleKeyDown = (event) => {
      // Блокируем Ctrl+P (печать), Ctrl+S (сохранить), F12 (DevTools), Ctrl+Shift+I
      if (
        (event.ctrlKey && (event.key === 'p' || event.key === 'P' || event.key === 's' || event.key === 'S')) ||
        event.key === 'F12' ||
        (event.ctrlKey && event.shiftKey && event.key === 'I')
      ) {
        event.preventDefault()
        event.stopPropagation()
        ElMessage.warning('Эта функция недоступна для конфиденциальных материалов')
        return false
      }
    }
